...
class QuickSearchOverlay extends ModalDialog.ModalDialog {
    constructor(applet, metadata) {
        ...
        // never changes size between states; the search region and the AI pane are
        // mutually exclusive surfaces (only the one that owns content is visible). The
        // follow-up composer lives at the bottom of the AI pane, directly under the
        // conversation scroll — nothing is expanded, so nothing gets pushed to the
        // bottom of the screen (issue #1/#7).
        ...
        // Added LAST (after the AI pane) so it always sits at the bottom of the shell:
        // under the results panel in Search, under the composer in AI chat. Hidden when
        // there is no content, so it never stands alone as an empty footer.
        try {
        ...
        // REBUILD: the AI conversation owns a DEDICATED scroll surface (aiResultsBox +
        ...
        try { this._aiScroll.add_actor(this.aiResultsBox); } catch (e) { try { this._aiScroll.add_child(this.aiResultsBox); } catch (e2) {} }
...
class QuickSearchApplet extends Applet.IconApplet {
    ...
    // REBUILD: explicit UI state classification (single source of truth for the shell).
    // Six states — idle-search / searching / search-results / ai-input / ai-thinking /
    // ai-answer — derived from existing state (mode + query + conversation), never a
    // second copy of it. The shell visibility in _syncShell() is driven by this.
    get _uiState() {
        const ov = this._overlay;
        if (this._mode === 'ai') {
            if (this._hasConversation()) {
                return (this._aiLoading || this._aiStreaming) ? 'ai-thinking' : 'ai-answer';
            }
            return 'ai-input';
        }
        const q = (ov && ov.getText) ? String(ov.getText() || '').trim() : '';
        if (q) return (this._sortedResults && this._sortedResults.length > 0) ? 'search-results' : 'searching';
        return 'idle-search';
    }

    // REBUILD: one structural visibility pass for the whole shell, driven by _uiState.
    // Empty-state rule: a section without content never takes visual space — hidden
    // actors contribute zero size, so the shell is compact in idle / ai-input and only
    // grows the panels that actually own content. The command bar (pill) is always
    // visible and never changes size between states.
    _syncShell() {
        const ov = this._overlay;
        if (!ov) return;
    ...
    _clearAIStateVisualOnly() {
        ...
        if (convMod && this._conversation) { try { convMod.cancelActive(this._conversation); } catch (e) {} }
        const ov = this._overlay;
        if (!ov || !ov.aiResultsBox) return;
        try { while (ov.aiResultsBox.get_n_children() > 0) ov.aiResultsBox.remove_child(ov.aiResultsBox.get_child_at_index(0)); } catch (e) {}
        this._mainRows = [];
        this._aiMsgActors = {};
        // keep _rows recomputed via _syncSelection/_render paths; ensure the AI surface
        // (scroll + pane) is hidden until normal Search renders take over
    ...
    _ensureScrollTracking() {
        const ov = this._overlay;
        if (!ov || !ov._aiScroll || this._aiScrollBound) return;
        try {
            const vbar = ov._aiScroll.get_vscroll_bar();
    ...
    _scheduleAIScroll(force) {
        ...
        this._aiScrollId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 20, () => {
            ...
            try {
                ...
                if (!ov2 || !ov2._aiScroll || !ov2._aiScroll.visible) return GLib.SOURCE_REMOVE;
                const vbar = ov2._aiScroll.get_vscroll_bar();
                const adj = vbar.get_adjustment();
                const upper = Number(adj.upper) || 0;
                const ps = Number(adj.page_size) || 0;
    ...
    _scrollAIMessageIntoView(id) {
        ...
        this._aiScrollId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 20, () => {
            ...
            try {
                ...
                const vbar = ov2._aiScroll.get_vscroll_bar();
                const adj = vbar.get_adjustment();
                const [, sy] = ov2._aiScroll.get_transformed_position();
            ...
            if (msg.role === 'user') {
                ...
                if (msg.status === 'streaming') {
                    ...
                    try {
                        ...
                        ov.aiResultsBox.add_child(errLbl);
                    } catch (e) {}
    ...
    close() {
        ...
        if (this._aiScrollAdjIds && this._aiScrollAdjIds.length > 0) {
            try {
                if (this._overlay && this._overlay._aiScroll) {
                    const vbar = this._overlay._aiScroll.get_vscroll_bar();
                    const adj = vbar.get_adjustment();
    ...
    // REBUILD: ONE hints bar at the bottom of the shell serves both modes — under the
    // results panel in Search, under the composer in AI chat. Hidden whenever there is
    // no content, so it never stands alone as an empty footer (empty-state rule).
    _updateHints() {
        const ov = this._overlay;
    ...
    renderResults(results) {
        ...
        if (display.length > 0) {
            ...
            // if suggestion rows still exist they carry the state — no empty panel
            const q = this._overlay && this._overlay.getText ? String(this._overlay.getText() || '').trim() : '';
            if (q && !(this._autoRows && this._autoRows.length > 0)) mainRows.push({ empty: true });
        }
