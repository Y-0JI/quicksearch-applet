...
// Boxed-free request body (2026-09-15): ONE implementation lives in soupTextReader.js
            ...
            return new Promise((resolve, reject) => {
                ...
                try {
                    ...
                    if (!_setRequestBodyBoxedFree(msg, 'application/json', body)) {
                        return reject(_attachStage(new Error('boxed-free request body unavailable'), 'request_build'));
                    }
    ...
    if (Soup && GLib) {
        ...
        return function soupStreamFetch(url, opts, onChunk, onDone) {
            try {
                ...
                // No Bytes fallback: boxed GLib.Bytes SEGVs Cinnamon at GC — abort instead.
                if (!_setRequestBodyBoxedFree(msg, 'application/json', body)) {
                    onDone(_attachStage(new Error('boxed-free request body unavailable'), 'request_build'));
                    return;
                }
                const appCancellable = opts.cancellable || null;
